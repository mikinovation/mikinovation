...
pub fn deserialize_create_todo(json: &JsonString) -> Result<CreateTodoDto, SerializationError> {
    serde_json::from_str(json).map_err(|e| SerializationError::Deserialize(e.to_string()))
}
...
pub fn create_todo_dto_to_domain(
    dto: CreateTodoDto,
) -> Result<CreateTodoInput, SerializationError> {
    let title = validate_title(dto.title).map_err(SerializationError::Validation)?;

    Ok(CreateTodoInput { title })
}

pub fn update_todo_dto_to_domain(
    id_str: &str,
    dto: UpdateTodoDto,
) -> Result<UpdateTodoInput, SerializationError> {
    let uuid = Uuid::parse_str(id_str)
        .map_err(|_| SerializationError::Validation("Invalid UUID format".to_string()))?;

    let title = if let Some(title_str) = dto.title {
        Some(validate_title(title_str).map_err(SerializationError::Validation)?)
    } else {
        None
    };

    let completed = dto.completed.map(Completed);
...
pub fn output_to_response(
    output: TodoOutput,
) -> Result<(axum::http::StatusCode, serde_json::Value), SerializationError> {
